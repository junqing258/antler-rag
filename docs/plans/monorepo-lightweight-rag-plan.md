...
- `users`：全局登录身份、密码摘要、状态；同一用户可加入多个租户。
...
- 浏览器会话：Bearer session token 标识用户；`X-Tenant-ID` 指定当前租户，后端必须校验有效 membership。
- 租户 API Key：Key 自身唯一绑定租户，忽略客户端伪造的其他租户 ID。
...
- 记录 embedding provider/model/version 或配置指纹；配置变化必须触发显式全量重建，不允许静默切换。
...
| 成员管理 | 邀请/创建用户、修改角色、禁用或移除 | tenant_admin |
| API Key | 创建、一次性复制、查看前缀/最后使用时间、吊销 | tenant_admin |
| 租户管理 | 创建、停用、选择目标租户 | platform_admin |
...
- 为原 `antler-rag` 的五类接口补足黑盒回归测试或固定请求/响应样例。
- 确认是否存在依赖旧 `/v1` 路由和全局 API Key 的客户端。
- 确认首版采用内置账号，而不是接入现有 SSO。
- 确认“一租户多知识库 + 自动默认知识库”的模型。
- 明确单文件大小、单次文件数、允许 MIME、默认 `top_k`、会话有效期和 API Key 默认权限。
...
- 将后端迁入 `apps/backend`，先保持现有行为和依赖版本范围。
...
- 实现 tenant、user、membership、session、tenant API Key 和 audit schema。
- 实现平台管理员 bootstrap、本地登录、注销、会话吊销与密码修改。
- 实现 `TenantContext`、RBAC 依赖和统一错误模型。
- 实现租户、成员、API Key 管理 API。
- 对 token/API Key 只保存摘要，并补充登录限速或基础失败退避。

退出标准：可以用两个租户、多个角色完成 API 级权限矩阵测试；伪造 `X-Tenant-ID` 无法进入非成员租户。
...
- 实现失败清理、删除状态和 reconciliation 命令。
- 根据 Phase 0 结果实现或取消旧 API 兼容层。

退出标准：原 RAG 能力回归通过；跨租户泄漏测试覆盖 list/retrieve/chat/get/delete 和伪造资源 ID，并全部通过。
...
- 完成单镜像、Compose、非 root 用户、只读应用目录和 `/data` 数据卷。
...
- 单元测试：文本提取、切块、密码/API Key 摘要、权限判断、路径构造、错误映射。
...
- 故障测试：空文档、不支持格式、超限文件、Chroma 写入失败、LLM 超时、删除中断、重启后 reconciliation。
...
- 管理 smoke：创建租户 -> 添加成员 -> 创建并吊销 API Key。
- 可访问性基础检查：键盘操作、表单 label、加载/空/错误状态、颜色对比。

### 9.4 根级 CI 顺序

1. 后端 format/lint/type check（若引入）与单元测试。
2. 前端 format check、lint、`vue-tsc --noEmit`、Vitest、build。
3. API 集成与租户隔离矩阵。
4. 生产 Docker 镜像构建及容器健康 smoke。
...
- API Key、session、密码、LLM 密钥不进入日志、错误响应或前端持久状态。
- 数据库和数据目录权限仅授予运行用户；容器以非 root 用户运行。
- 备份是整个 `/data` 的一致性快照；只备份 SQLite 或只备份 Chroma 都不能视为完整恢复点。
- 停用租户先阻断认证和写入，数据清理采用显式二次确认和可审计流程，不提供普通 API 的立即递归硬删除。
...
| 默认 embedding 模型首次下载 | 离线部署启动/首请求失败 | 镜像构建或部署阶段预热，并记录模型版本与缓存要求 |
...
2. 是否接受首版内置账号体系；若必须接现有 SSO，应在认证实现前替换方案。
3. 是否确认每租户支持多个知识库；本文默认“支持，并自动创建默认知识库”。
4. 目标部署是否保证单实例和本地持久卷；若首版就要求多副本，SQLite + 嵌入式 Chroma 不再合适。
5. 首发语言是否只要求简体中文/英文；本文默认这两种语言完整可用。
...
- 租户 API Key：默认不过期但支持设置过期时间；生产运维指南建议定期轮换。
...
- 安全配置、数据目录、单实例边界、embedding 变更和升级流程有运维文档。
